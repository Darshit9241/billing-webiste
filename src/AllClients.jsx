      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 sm:mb-8 space-y-4">
          ...
          <div className="relative group">
            <div className={`absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none ${isDarkMode ? 'text-slate-400' : 'text-gray-400'} group-focus-within:text-emerald-500 transition-colors duration-200`}>
            ...
            <input
              ...
              className={`w-full pl-12 pr-4 py-3 rounded-xl text-base ${isDarkMode ? 'bg-white/5 border-white/10 text-white placeholder-slate-400' : 'bg-white border-gray-200 text-gray-900 placeholder-gray-400'} border focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 shadow-sm transition-all duration-200`}
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery("")}
                className={`absolute inset-y-0 right-0 flex items-center pr-4 ${isDarkMode ? 'text-slate-400 hover:text-white' : 'text-gray-400 hover:text-gray-600'} transition-colors duration-200`}
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className={`flex items-center space-x-2 px-3 py-2 rounded-lg ${isDarkMode ? 'bg-white/5' : 'bg-gray-50'} border ${isDarkMode ? 'border-white/10' : 'border-gray-200'}`}>
              <div className="flex items-center space-x-1">
                {[
                  ...
                  { field: "totalAmount", label: "Total Value" },
                  { field: "pendingAmount", label: "Pending" },
                  { field: "lastOrderDate", label: "Last Order" }
                ].map(({ field, label }) => (
                  <button
                    key={field}
                    onClick={() => handleSort(field)}
                    className={`px-2 py-1 rounded-md text-xs font-medium transition-all duration-200 ${sortField === field
                        ? isDarkMode
            ...
            <button
              ...
              className={`px-3 py-2 rounded-lg text-sm font-medium flex items-center space-x-2 transition-all duration-200 ${showMergedClients
                  ? isDarkMode
                    ? 'bg-emerald-500/20 text-emerald-300'
                    : 'bg-emerald-100 text-emerald-700'
                  : isDarkMode
            ...
            >
              ...
              <span>{showMergedClients ? 'Including Merged' : 'Excluding Merged'}</span>
              {showMergedClients && mergedClientCount > 0 && (
                <span className="px-1.5 py-0.5 rounded-full text-xs bg-emerald-500/20 text-emerald-300">
                  {mergedClientCount}
                </span>
              )}
            </button>
