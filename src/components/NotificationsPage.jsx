...
const NotificationsPage = () => {
  ...
  return (
    <div className={`min-h-screen flex flex-col ${isDarkMode ? 'bg-slate-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <div className="flex-grow container mx-auto px-3 sm:px-4 py-4 sm:py-8">
        <div className="mb-4 sm:mb-8">
          <h1 className={`text-xl sm:text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
            Notifications
          </h1>
          <p className={`mt-1 sm:mt-2 text-sm sm:text-base ${isDarkMode ? 'text-slate-400' : 'text-gray-600'}`}>
            View all your important notifications in one place
          </p>
        </div>

        {/* Notification Tabs */}
        <div className={`flex w-full overflow-x-auto border-b mb-4 sm:mb-6 ${isDarkMode ? 'border-slate-700' : 'border-gray-200'}`}>
          <button
            onClick={() => setActiveTab('payments')}
            className={`py-2 sm:py-3 px-4 sm:px-6 font-medium text-xs sm:text-sm whitespace-nowrap relative ${
              activeTab === 'payments'
                ? isDarkMode
          ...
          >
            ...
            {bestSellers.length > 0 && (
              <span className="ml-1 sm:ml-2 inline-flex items-center justify-center px-1.5 sm:px-2 py-0.5 sm:py-1 text-xs font-bold leading-none text-blue-100 bg-blue-600 rounded-full">
                {bestSellers.length}
              </span>
            )}
          </button>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md overflow-hidden">
          {loading ? (
            <div className="flex items-center justify-center p-4 sm:p-8">
              ...
              <span className={`ml-3 text-sm sm:text-base ${isDarkMode ? 'text-slate-300' : 'text-gray-600'}`}>Loading notifications...</span>
            </div>
          ) : activeTab === 'payments' ? (
            notifications.length > 0 ? (
              <div>
                <div className={`px-4 sm:px-6 py-3 sm:py-4 ${isDarkMode ? 'bg-slate-700 text-white' : 'bg-gray-50 text-gray-900'} border-b ${isDarkMode ? 'border-slate-600' : 'border-gray-200'}`}>
                  <h2 className="font-medium text-sm sm:text-base">Clients with Pending Payments</h2>
                  <p className={`text-xs sm:text-sm ${isDarkMode ? 'text-slate-300' : 'text-gray-500'}`}>
                ...
                <div className="divide-y divide-gray-100 dark:divide-slate-700">
                  {notifications.map(client => (
                    <div key={client.id} className={`p-4 sm:p-6 ${isDarkMode ? 'hover:bg-slate-700/50' : 'hover:bg-gray-50'} transition-colors`}>
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
                        <div className="mb-3 sm:mb-0">
                          <Link 
                            to={`/order/${client.id}`}
                            className={`text-base sm:text-lg font-medium ${isDarkMode ? 'text-white hover:text-emerald-400' : 'text-gray-900 hover:text-emerald-600'}`}
                          >
                            {client.clientName || 'Unnamed Client'}
                          </Link>
                          <div className="mt-1 flex flex-col xs:flex-row flex-wrap gap-y-1 gap-x-4">
                        ...
                        <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-start">
                          <div className="bg-red-100 text-red-800 text-xs font-medium px-2 sm:px-3 py-0.5 sm:py-1 rounded-full dark:bg-red-900/30 dark:text-red-300">
                            {calculateDaysOverdue(client)} days overdue
                          </div>
                          <p className={`text-base sm:text-lg font-medium sm:mt-2 ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>
              ...
              <div className="p-4 sm:p-8 text-center">
                <svg xmlns="http://www.w3.org/2000/svg" className={`h-12 w-12 sm:h-16 sm:w-16 mx-auto mb-3 sm:mb-4 ${isDarkMode ? 'text-slate-600' : 'text-gray-300'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <h3 className={`text-base sm:text-lg font-medium mb-1 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                  No Pending Payments
                </h3>
            ...
            bestSellers.length > 0 ? (
              <div>
                <div className={`px-4 sm:px-6 py-3 sm:py-4 ${isDarkMode ? 'bg-slate-700 text-white' : 'bg-gray-50 text-gray-900'} border-b ${isDarkMode ? 'border-slate-600' : 'border-gray-200'}`}>
                  <h2 className="font-medium text-sm sm:text-base">Top Performing Clients</h2>
                  <p className={`text-xs sm:text-sm ${isDarkMode ? 'text-slate-300' : 'text-gray-500'}`}>
                ...
                <div className="divide-y divide-gray-100 dark:divide-slate-700">
                  {bestSellers.map((client, index) => (
                    <div key={client.id} className={`p-4 sm:p-6 ${isDarkMode ? 'hover:bg-slate-700/50' : 'hover:bg-gray-50'} transition-colors`}>
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
                        <div className="mb-3 sm:mb-0 flex items-start sm:items-center">
                          <div className={`w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-full mr-3 sm:mr-4 flex-shrink-0 ${
